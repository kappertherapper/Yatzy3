...
async function readFile() {
  let fileContent = await fs.promises.readFile("players.json", {
    encoding: "utf-8",
  });
...
async function updatePlayerScore(name, field, point, total) {
  ...
  Object.defineProperty(currentDB[playerIndex].scoreVals, field, {
    value: point,
    writable: true,
  });

  Object.defineProperty(currentDB[playerIndex].scoreVals, "total", {
    value: total,
    writable: true,
  });
...
async function initPlayersJSON() {
  try {
    ...
    fs.writeFile("players.json", JSON.stringify([]), (err) => {
